...
/* 改写自 mongoos.c -> mg_connect_http()，支持二进制数据的发送
* 2018-11-27  黄汉毅
*/
struct mg_connection *mg_connect_http_ext(struct mg_mgr *mgr,
        mg_event_handler_t ev_handler,
        const char *url,
        const char *extra_headers,
        const char *post_data, int _body_len)
{
    struct mg_connect_opts opts;
    memset(&opts, 0, sizeof(opts));
    ...
    //    "\r\n%.*s%s\r\n%s",
    //    post_data == NULL ? "GET" : "POST", path, addr,
    //    post_data == NULL ? 0 : _body_len, (int)auth.len,
    //    (auth.buf == NULL ? "" : auth.buf),
    //    extra_headers == NULL ? "" : extra_headers,
    //    post_data == NULL ? "" : post_data);

    int len_head = 100 + strlen(path) + strlen(addr) + auth.len + strlen(extra_headers);
    char *head = malloc(len_head);
    memset(head, 0, len_head);
    unsigned long body_len = _body_len;
    snprintf(head, len_head, "POST %s HTTP/1.1\r\nHost: %s\r\nContent-Length: %" SIZE_T_FMT "\r\n%.*s%s\r\n",
             path, addr, body_len,
             (int)auth.len, (auth.buf == NULL ? "" : auth.buf),
             extra_headers == NULL ? "" : extra_headers);

    mg_send(nc, head, strlen(head));

    free(head);

    mg_send(nc, post_data, _body_len);
